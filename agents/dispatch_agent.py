...
Отправка в Telegram сегодня работает в режиме DRY RUN (печатает, что было
бы отправлено, и кому) -- потому что: (а) у нас ещё нет токена бота от
Леонида, (б) сама песочница Claude не достаёт напрямую до api.telegram.org
(проверено curl'ом -- 403 на уровне прокси), так что даже с токеном
отправка отсюда не заработает -- этот код рассчитан на то, что будет
исполняться на VPS, где обычный внешний интернет есть. Как только появится
токен, send_via_telegram() ниже -- готовая точка замены dry-run на реальный
requests.post к Bot API.
"""
...
def send_via_telegram(message: str, recipients: list[Recipient], bot_token: str | None) -> dict:
    """
    DRY RUN пока нет токена / нет доступа к api.telegram.org из этой сессии.
    Возвращает то, что было бы отправлено, вместо реальной отправки, и
    честно помечает dry_run=True, а не имитирует успешную отправку.
    """
    ...
    for r in recipients:
        if bot_token is None or r.telegram_chat_id is None:
            ...
            continue
        # Реальная отправка (заготовка, не вызывается пока bot_token is None):
        #   import requests
        #   requests.post(
        ...
        #       timeout=10,
        #   )
        result["sent_to"].append({"recipient": r.label, "status": "would_send"})
