...
def format_consensus_note(agree: bool | None, detail: str) -> str | None:
    """
    27 августа, по отзыву "брокера" (см. project doc) -- расхождение
    независимой сверки раньше считалось, но реально было видно только в
    консоли сервера, получатель алерта его вообще не видел. Это была прямая
    информация об уверенности в сигнале (свежий, ещё не подтверждённый
    вторым методом экстремум), которая просто терялась.

    agree=None -- сверка была недоступна в этом прогоне (см. ValueError из
    find_fractal_swing_extremes) -- detail тогда несёт текст ошибки.
    agree=True/False -- обычный ConsensusResult.detail.

    Специально НЕ делает сверку блокирующей (это отдельное решение, см.
    "Открытые вопросы" в README) -- только формулирует то же самое честно
    словами для человека, а не только для консоли.

    20 сентября 2026 (облегчение сообщений, прямой запрос Леонида): при
    agree=True возвращает None, а не текст подтверждения. Это НЕ откат
    решения 27 августа "показывать сверку получателю" -- то решение было
    про то, чтобы не ПРЯТАТЬ расхождение (agree=False) или недоступность
    (agree=None), то есть настоящую неопределённость. "Всё сошлось" --
    не неопределённость, а рутинное подтверждение; для лёгкого сообщения
    это шум на каждой отправке. Расхождение и недоступность по-прежнему
    возвращают текст всегда, без исключений -- ни один настоящий caveat
    не должен потеряться молча (регламент, раздел 2).
    """
    if agree is None:
        return f"🔍 Независимая сверка структуры: недоступна -- {detail}"
    if agree:
        return None
    return (
        f"🔍 Независимая сверка структуры: РАСХОЖДЕНИЕ -- {detail} "
        f"Экстремум ещё не подтверждён вторым методом -- отнеситесь к сигналу с осторожностью."
    )
