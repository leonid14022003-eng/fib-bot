...
=================================================
ВНИМАНИЕ (21 сентября 2026): этот файл считает на МАЛОЙ выборке (15 инструментов
FMP, ~33 сигнала) и без контрольной группы -- вывод "edge отрицательный" из
13 сентября по нему был ошибкой (стандартная ошибка среднего при n=33 ≈0.17-0.2R,
то есть -0.20R неотличимо от нуля). Корректная проверка на ~650 инструментах с
контролем случайными входами и делением на обучение/проверку -- research/
(см. раздел "Исследование методики" в CLAUDE.md). Этот файл оставлен как есть:
на нём завязаны тесты (в т.ч. no-lookahead) и он остаётся эталоном, с которым
быстрый движок research/ сверен побитово. Известные ограничения: (1) считает
глобальные экстремумы окна, а не "oldest unbroken" как screener.py (на практике
совпадают); (2) убыток в R не ограничен снизу, а на уровне 1.0 риск-единица
близка к нулю -- отдельные сигналы могут раздувать среднее.
